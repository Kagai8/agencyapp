...
<html lang="en">
  <head>
    <meta charset="utf-8">
    <title>Done Payment Plan Receipt For PP{{$payment_plan->id }}</title>
    ...
    <main>
      <div id="details" class="clearfix">
        <div id="client">
          ...
          <h2 class="name">{{ $payment_plan->customer->customer_name }}</h2>
          <div class="address">{{ $payment_plan->customer->customer_phone_no }}</div>
          <div class="email"><a href="mailto:john@example.com">{{$payment_plan->customer->customer_email }}</a></div>
        </div>
        <div id="invoice">
          <h1>RECEIPT FOR INSTALLMENTS FOR PP{{$payment_plan->id }}</h1>
      ...
      <table border="0" cellspacing="0" cellpadding="0">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="desc"> {{$item->installment}}</td>
            <td class="qty">{{\Carbon\Carbon::parse($item->created_at )->format('d F Y')}}</td>
            <td class="total"> {{ $item->installment }}</td>
          </tr>
          @endforeach
        </tbody>
        <tfoot>
          ...
          <tr>
            ...
            <td style="text-underline-position: 20px">KSH  {{ $totalInstallments}}</td>
          </tr>
        </tfoot>
      </table>
